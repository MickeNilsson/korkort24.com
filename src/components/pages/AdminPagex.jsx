...
export default function AdminPage(props) {
    ...
    async function getBookings() {
        ...
        let response = await fetch('https://korkort24.com/api/times/');

        const allTimes = await response.json();

        const bookedTimes = allTimes.filter(time => time.studentid);

        setBookings(bookedTimes);
        // response = await fetch('https://korkort24.com/api/students/');
    ...
    return (
        <>
                            ...
                            <table class="table table-striped table-hover">
                                ...
                                <tbody>
                                    {bookings.map(booking => (
                                        <tr><td>{booking.studentid}</td><td>{booking.from.substring(0, 10)}</td><td>{booking.from.substring(11, 16)} - {booking.to.substring(11, 16)}</td></tr>
                                    ))}
                                </tbody>
            ...
            <Modal show={showModal} onHide={handleClose} onHide={() => setShowConfirmationModal(false)}>
